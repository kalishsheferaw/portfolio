/* ==========================================================
   EDIT ME. This is the only file you need to change to switch
   on email, GitHub, CV, repo links and the contact form.
   Anything left as "" stays hidden on the site.
   ========================================================== */
window.SITE = {
  email:        "kalishshiferaw@gmail.com",
  github:       "",  // "https://github.com/your-username"
  cv:           "assets/docs/Kalish-Shiferaw-CV.pdf",  // the button appears only once this PDF is uploaded to assets/docs/
  mortgageRepo: "",  // GitHub repo with SSIS package, SQL scripts and README
  olistRepo:    "",  // GitHub repo with the .pbix, README and screenshots
  olistLive:    "",  // Power BI "Publish to web" link, if you publish one
  formEndpoint: ""   // Formspree endpoint, e.g. "https://formspree.io/f/abcdwxyz"
};
